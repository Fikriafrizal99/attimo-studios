...
10. **`PHASE6_ADMIN_WORKFLOW.md`**  
    Phase 6 operations overview, order pipeline, customer/order workspaces, publish-readiness visibility, workflow controls, and activity trail.

11. **`PRODUCTION_P0_RUNBOOK.md`**  
   Supabase, Better Auth, tenant-isolation, storage, domain, and device verification baseline.

12. **`SELF_HOST_DOCKER.md`**  
   Self-host deployment on a single Ubuntu/Docker server.

13. **`UPSTREAM_LICENSE_STATUS.md`**  
...
- Commercial production status changes remain explicit operator actions.
- The commerce workflow does not duplicate or replace the centralized wedding publish-readiness validator.
- Order activity history is generated by the database and is read-only to browser roles.
...
Phase 7  Template Catalog Scaling           NEXT
...
During Phases 7–8, each meaningful implementation must keep the relevant verification chain green:
...
| Phase 6 Admin Workflow | **Implemented; environment/E2E verification deferred to Phase 9** |
