...
import type { TemplateDefinition, TemplatePerformanceBudget, TemplateVisualTier } from "@/templates/types";
...
function validBudget(budget: TemplatePerformanceBudget): boolean {
  return Number.isFinite(budget.experienceJsKb) && budget.experienceJsKb > 0 &&
    Number.isFinite(budget.mediaKb) && budget.mediaKb > 0 &&
    Number.isFinite(budget.maxDevicePixelRatio) && budget.maxDevicePixelRatio >= 1 && budget.maxDevicePixelRatio <= 2;
}
...
function defineTemplate(definition: TemplateDefinition): TemplateDefinition {
  ...
  if (!validBudget(definition.performance.budget)) throw new Error(`Template must provide a valid performance budget: ${definition.id}`);
...
const BUDGET_2D_LIGHT = { experienceJsKb: 90, mediaKb: 6500, maxDevicePixelRatio: 1 } as const;
const BUDGET_2D_RICH = { experienceJsKb: 150, mediaKb: 7500, maxDevicePixelRatio: 1 } as const;
const BUDGET_25D = { experienceJsKb: 190, mediaKb: 8000, maxDevicePixelRatio: 1.5 } as const;
const BUDGET_3D = { experienceJsKb: 240, mediaKb: 9000, maxDevicePixelRatio: 1.5 } as const;
...
export const TEMPLATE_REGISTRY: Readonly<Record<string, TemplateDefinition>> = Object.freeze({
  "classic-001": defineTemplate({
    ...
    performance: { renderingMode: "dom", motionLevel: "rich", mobileProfile: "full", reducedMotionFallback: true, budget: BUDGET_2D_RICH },
  }),
  "editorial-001": defineTemplate({
    ...
    contentSchemaVersion: 1, sectionContract: FULL_SECTION_CONTRACT,
    performance: { renderingMode: "dom", motionLevel: "light", mobileProfile: "full", reducedMotionFallback: true, budget: BUDGET_2D_LIGHT },
  }),
  ...
  "paper-cut-001": defineTemplate({
    ...
    contentSchemaVersion: 1, sectionContract: FULL_SECTION_CONTRACT,
    performance: { renderingMode: "hybrid", motionLevel: "rich", mobileProfile: "adaptive", reducedMotionFallback: true, budget: BUDGET_25D },
  }),
  ...
  "clay-001": defineTemplate({
    ...
    contentSchemaVersion: 1, sectionContract: FULL_SECTION_CONTRACT,
    performance: { renderingMode: "webgl", motionLevel: "immersive", mobileProfile: "adaptive", reducedMotionFallback: true, budget: BUDGET_3D },
  }),
